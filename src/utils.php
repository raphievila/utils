...
class Utils
{
    ...
    private static $company = 'Your Company Name';
    private static $phone = '555-555-5555';
    private static $email = 'anyone@example.com';
    private static $url = 'http://example.com';

    private static $approvedDomains = array(
        'localhost',
        "127\.0\.0",
    );
        ...
        if (count($matches) > 0) {
            if (is_array($matches[0]) && count($matches[0]) > 0) {
                ...
                if (isset($matches[0][0])) {
                    $browser->full = $this->split_enchancement($matches[0][0]);
                }
                if (isset($matches[1][0])) {
                    $browser->compatibility = $this->split_enchancement($matches[1][0]);
                }
    ...
    //JUST FOR SEMANTICS
    public function echo_raw_string($string)
    {
        $this->echo_array($string);
    }
    ...
    public function user_generator($string, $string2 = false)
    {
        ...
        $words = explode(' ', strtolower($removeSymbols));
        $user = array();
        ...
        if ($string2) {
            ...
            if (isset($br[1])) {
                $nuser .= str_replace($replacing, '', ucfirst(strtolower($br[1])));
            }
        }
    ...
    public static function recursive_map_to_xml($map, $args = false)
    {
        ...
        $showHeader = true;
        if (!self::CHECKMAP($map)) {
            throw new Exception('First argument requires an object or array', 500);
        }
